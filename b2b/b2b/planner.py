...
def make_plan(tracks,tempo=128,bars=16,direction='',use_ai=False):
    ...
    if use_ai:
        ...
        prompt={'task': 'Program a coherent 4/4 house DJ set. Choose the order, and choose one supplied 8/16-bar transition option per consecutive pair. Give a concise public decision summary for the set and each handoff. For each handoff, name the incoming track and explain your choice in first person, with the taste and dry wit of a resident DJ. Vary the phrasing; do not repeat a fill-in-the-blank template. Add a second short sentence about the selected blend if useful. Keep each summary under 45 words, distinguish estimated evidence from listening, and do not provide hidden reasoning or an internal deliberation trace. Use as many suitable tracks as possible once each; exclude stylistic outliers. Never invent cues. Each middle track needs 8 seconds after its incoming handoff before its outgoing transition. The audio engine uses the tested continuous-gain bass handoff; do not invent effects or curves.',
