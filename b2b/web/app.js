...
async function arm(index,currentIndex,my,preview){if(!active||my!==generation)return;const a=decks[currentIndex],b=decks[1-currentIndex],e=plan.transitions[index];if(!e){$('#autoTitle').textContent='Last record';$('#autoDetail').textContent='Enjoy the rest of the track.';active=false;$('#takeover').hidden=true;$('#auto').hidden=false;$('#tempo').disabled=false;return}
 ...
 $('#autoTitle').textContent=preview?'Previewing the transition':'Transition scheduled';$('#autoDetail').textContent=`${e.bars} bars → ${b.track.title} · ${e.reason}`;
...
async function edit(id){
 ...
 if(active)takeover();await unlock();if(d.track?.id!==id||!d.buffer)await d.load(t,masterTempo);
...
$('#editForm').onclick=e=>{const field=e.target.dataset.marker;if(!field)return;const v=inspection();if(!v?.d.buffer)return;const seconds=v.d.position()*v.d.ratio;if(field==='gridOffset'&&seconds>30){toast('Choose beat one within the first 30 seconds. Use the phrase marker for a later section.');return;}$('#editForm').elements[field].value=seconds.toFixed(3);inspectCache=null;};
$('#editForm').onsubmit=e=>{e.preventDefault();safe(async()=>{
 ...
 try{await api('track/'+id,{...values,reviewed:true},'PUT');plan=null;$('#editor').hidden=true;await refresh();for(const d of decks.filter(d=>d.track?.id===id))await d.load(track(id),masterTempo);toast('Markers saved. The waveform and deck now use your confirmed grid.');}finally{button.disabled=false;}
 });};
