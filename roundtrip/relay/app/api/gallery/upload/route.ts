...
export async function POST(req:Request){
 ...
 try{
  ...
  if(!(file instanceof File)||file.size>3000000||typeof id!=='string'||!/^[a-z0-9]{2,32}$/.test(id)||typeof label!=='string'||label.length>30||typeof summary!=='string'||summary.length>500||!Number.isFinite(created))throw Error('Invalid export.');
  if(await getDb().prepare('SELECT 1 FROM gallery_revisions WHERE id=?').bind(id).first())return json({ok:true});
  ...
  const objectKey='portrait/'+id+'.jpg';await mediaBucket().put(objectKey,bytes,{httpMetadata:{contentType:'image/jpeg'}});
  await getDb().prepare('INSERT INTO gallery_revisions VALUES(?,?,?,?,?,?)').bind(id,'portrait',label,summary,objectKey,created).run();return json({ok:true},201);
